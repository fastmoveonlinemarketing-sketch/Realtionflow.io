...
.mm-sec{padding:clamp(80px,10vw,150px) 0;position:relative;overflow:hidden;background:linear-gradient(180deg,#fff, var(--bg-alt))}
.mm-wrap{max-width:var(--maxw);margin:0 auto;padding:0 24px}
.mm-center{text-align:center;max-width:780px;margin:0 auto}
...
.mm-part{margin-top:clamp(54px,8vw,110px)}
.mm-phead{max-width:660px}
...
/* ROUTING */
.mm-pipe{display:grid;grid-template-columns:1fr auto auto auto 1fr;align-items:center;gap:12px;margin-top:8px}
...
@media(max-width:760px){.mm-pipe{grid-template-columns:1fr}.mm-flow{flex-direction:row;padding:4px 0}}
...
def meter(v):
    return '<span class="mm-meter">'+''.join('<i class="on"></i>' if i<v else '<i></i>' for i in range(3))+'</span>'
...
for dim,vals in dims:
    ...
    for i,v in enumerate(vals):
        ...
        cells+='<div class="mm-cell%s">%s</div>'%(cls,meter(v))
    mrows+='<div class="mm-rowh">%s</div>%s'%(dim,cells)
...
models_data=[('gpt','Der Allrounder','Stark im Reasoning und bei vielschichtigen Aufgaben.'),
             ('claude','Der Tiefdenker','Brilliert bei langen Dokumenten und feinen Nuancen.'),
             ('gemini','Der Multimodale','Versteht Bilder, Tabellen und Medien nativ.'),
             ('mistral','Der Sprinter','Extrem schnell & günstig – ideal für Massenaufgaben.'),
             ('byom','Euer Eigenes','On-premise, EU, voll unter eurer Kontrolle.')]
models='<div class="mm-models mm-reveal">'+''.join(
 '<div class="mm-model"><div class="ml" style="background:%s">%s</div><b>%s</b><div class="role">%s</div><p>%s</p></div>'%(LG[k],SH[k],NAME[k],role,desc)
 for k,role,desc in models_data)+'</div>'
...
problem='<div class="mm-dark mm-reveal"><div class="mm-phead"><span class="mm-kick" style="color:#ff8b8b">Das Risiko</span><h3 class="mm-ph2" style="color:#fff">Nur ein Modell zu nutzen, ist 2026 ein teurer Fehler.</h3><p class="mm-psub" style="color:#9aa6bd">Wer alles auf einen Anbieter setzt, kauft sich dessen Schwächen mit ein – und gibt Qualität, Sicherheit und Geld freiwillig auf.</p></div><div class="mm-prob-grid">'+''.join('<div class="mm-prob"><span class="tagx">Risiko</span><div class="pi">%s</div><h4>%s</h4><p>%s</p></div>'%(ic,t,d) for t,d,ic in probs)+'</div></div>'

# routing
modelbar=''.join('<span class="mm-rm" data-m="%s"><span class="d" style="background:%s"></span>%s</span>'%(k,LG[k],NAME[k]) for k in order)
routing='''<div class="mm-dark mm-reveal"><div class="mm-phead"><span class="mm-kick" style="color:#7da9ff">Smart-Select</span><h3 class="mm-ph2" style="color:#fff">Die richtige Wahl trifft Relationflow automatisch.</h3><p class="mm-psub" style="color:#9aa6bd">Ihr schreibt einfach eure Aufgabe. Smart-Select analysiert sie und leitet sie an das Modell, das sie am besten löst &ndash; in Echtzeit.</p></div>
<div class="mm-pipe go" id="mmPipe">
...
<div class="mm-modelbar" id="mmModelbar">'''+modelbar+'</div></div>'
...
scen='<div class="mm-scen mm-reveal">'+''.join(
 '<div class="mm-sc"><div class="si">%s</div><div class="task">%s</div><div class="arrow">wird gelöst von</div><span class="win"><span class="d" style="background:%s"></span>%s</span><p>%s</p></div>'%(ic,task,LG[k],NAME[k],why)
 for task,k,why,ic in scen_data)+'</div>'
...
rows=[metric('Ergebnis-Qualität',('Begrenzt',55),('Optimal',96)),
      metric('Ausfallsicherheit',('Single Point of Failure',30),('Automatischer Fallback',99)),
      metric('Kosten-Effizienz',('Oft zu teuer',45),('Richtiges Modell pro Task',88)),
      metric('Zukunftssicherheit',('Lock-in',35),('Jederzeit erweiterbar',97))]
one_html=''.join(r[0] for r in rows); multi_html=''.join(r[1] for r in rows)
compare='<div class="mm-cmp mm-reveal"><div class="mm-col one"><div class="ch"><span class="tag">Nur 1 Modell</span>Single-Model</div>'+one_html+'</div><div class="mm-col multi"><div class="ch"><span class="tag">Multi-Modell</span>Relationflow</div>'+multi_html+'</div></div>'

# numbers
nums=[('40','%','bessere Ergebnisse','durch das jeweils beste Modell'),
      ('0','','Vendor Lock-in','jederzeit Modelle wechseln'),
      ('99.9','%','Verfügbarkeit','dank automatischem Fallback'),
      ('45','%','geringere Kosten','richtiges Modell pro Aufgabe')]
...
# closing
closing='<div class="mm-dark mm-close mm-reveal"><p class="big">Mit nur einem KI-Modell zu arbeiten heißt, freiwillig auf <span>Qualität, Sicherheit und Geld</span> zu verzichten.</p><p class="sub">Relationflow gibt euch alle f&uuml;hrenden Modelle &ndash; plus eure eigenen &ndash; in einer sicheren, DSGVO-konformen Oberfl&auml;che.</p><div class="cta"><a href="#" class="btn btn-light">Kostenlos starten</a><a href="#" class="btn btn-outline-light">Demo buchen</a></div></div>'

# intro
intro='''<div class="mm-wrap mm-center mm-reveal"><span class="mm-ey"><i></i>Multi-Modell &middot; Tiefenanalyse</span>
...
<div class="nd" style="left:77px;top:42px;background:linear-gradient(135deg,#5b8def,#1e40af);width:50px;height:50px;font-size:.66rem;animation-delay:0s">Hub</div>
...
SECTION = ('<section class="mm-sec">'
 + intro
 + '<div class="mm-wrap mm-part">'+problem+'</div>'
 + '<div class="mm-wrap mm-part"><div class="mm-phead mm-reveal"><span class="mm-kick">Die L&ouml;sung</span><h3 class="mm-ph2">F&uuml;nf Spezialisten statt einem Generalisten.</h3><p class="mm-psub">Jedes Modell hat eine Superkraft. Multi-Modell hei&szlig;t: ihr nutzt sie alle &ndash; je nach Aufgabe.</p></div>'+models+'</div>'
 + '<div class="mm-wrap mm-part"><div class="mm-phead mm-reveal"><span class="mm-kick">Der Beweis</span><h3 class="mm-ph2">Kein Modell f&uuml;hrt &uuml;berall.</h3><p class="mm-psub">Diese Matrix zeigt es schwarz auf wei&szlig;: Je nach Aufgabe gewinnt ein anderes Modell. Genau deshalb braucht ihr alle.</p></div>'+matrix+'<p class="mm-mcap">In <b>jeder</b> Zeile gewinnt ein anderes Modell &ndash; ein einzelnes Modell ist nie &uuml;berall vorne.</p></div>'
 + '<div class="mm-wrap mm-part">'+routing+'</div>'
 + '<div class="mm-wrap mm-part"><div class="mm-phead mm-reveal"><span class="mm-kick">In der Praxis</span><h3 class="mm-ph2">Dieselbe Plattform &ndash; immer das passende Modell.</h3><p class="mm-psub">Konkrete Aufgaben aus dem Arbeitsalltag und das Modell, das sie am besten l&ouml;st.</p></div>'+scen+'</div>'
 + '<div class="mm-wrap mm-part"><div class="mm-phead mm-reveal"><span class="mm-kick">Der direkte Vergleich</span><h3 class="mm-ph2">Single-Model gegen Multi-Modell.</h3></div>'+compare+'</div>'
 + '<div class="mm-wrap mm-part"><div class="mm-phead mm-center mm-reveal" style="max-width:680px"><span class="mm-kick">Der Nutzen in Zahlen</span><h3 class="mm-ph2">Was Multi-Modell konkret bringt.</h3></div>'+numbers+'</div>'
 + '<div class="mm-wrap mm-part">'+closing+'</div>'
 + '</section>')
...
anchor='</section>\n<style>\n/* ===== Alle Features — Alles-drin-Stil ===== */'
assert anchor in s, 'anchor not found'
s=s.replace(anchor, '</section>\n'+CSS+'\n'+SECTION+'\n'+JS+'\n<style>\n/* ===== Alle Features — Alles-drin-Stil ===== */', 1)
open(f,'w').write(s)

import re
print('mm-sec:',s.count('class="mm-sec"'),'mm-reveal:',s.count('mm-reveal'),'best cells:',s.count('mm-cell best'))
